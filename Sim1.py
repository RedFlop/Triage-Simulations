...
def run_simulation(seed):
    ...
    average_queue_length_before_420 = (sum(length for time, length in clinic.queue_lengths if time < 420) / len([time for time, length in clinic.queue_lengths if time < 420])
    if [time for time, length in clinic.queue_lengths if time < 420] else 0)
